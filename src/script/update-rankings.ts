...
import {
  getHeaderHtmlDivElement,
  getEntryHtmlDivElement,
  getOverlayHtmlDivElement
} from './utils/get-html-div-elements.js';
import { getFlagImgHtmlDivElement } from './utils/get-html-image-elements.js';

// do the stuff xdd
export async function updateRankings(): Promise<void> {
  try {
    const response: Readonly<Response> = await fetch(API_URL);
    const streamersUnsorted = (await response.json()) as readonly Streamer[];
    const streamersSorted: readonly Streamer[] = [...streamersUnsorted].sort(
      (streamer1, streamer2) => streamer2.subs - streamer1.subs
    );
    ...
    streamersSorted.forEach((streamer, index) => {
      ...
      if (streamer.name === CUTEDOG) entryHtmlDivElemen.classList.add('glass-effect');
